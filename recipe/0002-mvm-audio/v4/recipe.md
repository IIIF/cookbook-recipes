...
Audio resources are contained inside a [Timeline](https://iiif.io/api/presentation/4.0/#timeline), which is required to have a `duration` property with the time, in seconds, of the audio resource to be included in the timeline. The value of the `duration` property [must be a positive number](https://iiif.io/api/presentation/4.0/model/#duration), such as `1985` or `1985.024`.
...
The `type` of the resource is "Audio" but viewers should accept "Sound" as a synonym for Audio to improve compatibility with previous versions. For more details see the [type definition](https://iiif.io/api/presentation/4.0/model/#type). 
